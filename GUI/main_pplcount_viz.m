...
if(strcmp(sceneRun,'GUI_Setup'))
    ...
    fprintf("wall: aaa:");
    wall = hSetup.wall;
    wall_k1 = hSetup.wall_k1;
    wall_b1 = hSetup.wall_b1;
    wall_k2 = hSetup.wall_k2;
    wall_b2 = hSetup.wall_b2;
...
for iFig = 1:5
    ...
    if(strcmp(figureTitles{iFig},'Point Cloud'))
        ...
        if (wall_k1 && wall_b1 && ~wall_k2 && wall_b2)
            line(trackingAx, [-6 6], [-6*wall_k1+wall_b1,6*wall_k1+wall_b1],'Color', 'blue', 'LineWidth', 3);
        elseif (wall_k2 && wall_b2 && ~wall_k1 && ~wall_b1)
            line(trackingAx, [-6 6], [-6*wall_k2+wall_b2,6*wall_k2+wall_b2],'Color', 'blue', 'lineWidth', 3);
        elseif (wall_k2 && wall_b2 && wall_k1 && wall_b1)
            cross_x = (wall_b2 - wall_b1) /2;
            cross_y = cross_x + wall_b1;
...
%                            pointCloud(2,:) = pointCloud(2,:)*pi/180;
                            ...
                            inRangeInd = (pointCloud(1,:) > 1) & (pointCloud(1,:) < 6) & ...
                                ...
                                ((posAll(2,:) - posAll(1,:)*wall_k1 - wall_b1 >= 0 & (wall_b1 <= 0)) | ((posAll(2,:) - posAll(1,:)*wall_k1 - wall_b1 <= 0) & (wall_b1 >= 0))) & ...
                                ((posAll(2,:) - posAll(1,:)*wall_k2 - wall_b2 >= 0 & (wall_b2 <= 0)) | ((posAll(2,:) - posAll(1,:)*wall_k2 - wall_b2 <= 0) & (wall_b2 >= 0)));
...
%}
                    ...
                    case 7
                        ...
                        inRangeInd1 = ((S(1,:) > scene.areaBox(1)) & (S(1,:) < (scene.areaBox(1) + scene.areaBox(3))) & ...
                            ...
                            ((S(2,:) - S(1,:)*wall_k1 - wall_b1 >= 0 & (wall_b1 <= 0)) | ((S(2,:) - S(1,:)*wall_k1 - wall_b1 <= 0) & (wall_b1 >= 0))) & ...
                            ((S(2,:) - S(1,:)*wall_k2 - wall_b2 >= 0 & (wall_b2 <= 0)) | ((S(2,:) - S(1,:)*wall_k2 - wall_b2 <= 0) & (wall_b2 >= 0))));
